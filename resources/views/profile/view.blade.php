<section>
    ...
    <div class="crm-rightsidebar-header">
        <div class="crm-rightsidebar-header-left">
            <div class="crm-rightsidebar-header-title">{{ __('Profile Information') }}</div>
            <div class="crm-rightsidebar-header-text">{{ __("Your account's profile information and email address.") }}</div>
        </div>
        ...
        <div class="space-y-4">
            <div class="row">
                <div class="col-md-12 py-3">
                    <div class="text-center mb-3 d-flex justify-content-sm-center">
                        @if (auth()->user()->avatar)
                            <img src="{{ getImage(getFilePath('userProfile') . '/' . auth()->user()->avatar, getFileSize('userProfile')) }}"
                            alt="@lang('Profile')" class="profile-user-img rounded-circle" height="150" width="150">
                        @else
                        <img src="{{ asset('/img/default-avatar.png') }}"
                            alt="@lang('Profile')" class="profile-user-img rounded-circle" height="150" width="150">
                        @endif
                    </div>
                    <h3 class="profile-username text-center font-bold">{{ Auth::user()->name }}</h3>
            ...
            <div class="row px-3">
                <div class="col-md-6 py-2">
                    <label class="text-muted py-1"><i class="fas fa-map-marker-alt mr-1"></i> {{ __('Country') }}</label>
                    <p class="font-bold">{{ Auth::user()->country->name }}</p>
                </div>

                <div class="col-md-6 py-2">
                    <label class="text-muted py-1"><i class="fas fa-phone mr-1"></i> {{ __('Phone') }}</label>
                    <p class="font-bold">{{ Auth::user()->phone }}</p>
                </div>

                <div class="col-md-6 py-2">
                    <label class="text-muted py-1"><i class="fas fa-briefcase mr-1"></i> {{ __('Business Unit') }}</label>
                    <p class="font-bold">{{ Auth::user()->businessUnit->name }}</p>
                </div>

                <div class="col-md-6 py-2">
                    <label class="text-muted py-1"><i class="fas fa-circle-info mr-1"></i> {{ __('Status') }}</label>
                    <p class="font-bold">{{ ucfirst(strtolower(Auth::user()->status)) }}</p>
                </div>
            </div>

            <div class="row">
                <div class="gap-4 py-4 text-center">
                    <a class="crm-green-btn crm-show-right-side-bar d-block" href="#" data-type="edit-profile">{{ __('Edit Profile') }}</a>
                </div>
            </div>
        </div>
    </div>
