<section>
    <div class="crm-rightsidebar-header">
        <div class="crm-rightsidebar-header-left">
            <div class="crm-rightsidebar-header-title">{{ __('Change Password') }}</div>
            <div class="crm-rightsidebar-header-text">{{ __("Update your account's profile information and email address.") }}</div>
        </div>
    ...
    <div class="crm-rightsidebar-body form-row col-md-12">
        <div class="space-y-6">
            <form id="changePasswordForm" action="{{ route('profile.changePassword') }}">
                ...
                <div class="py-2">
                    <x-input-label for="currentPassword" :value="__('Current Password')" />
                    <x-text-input id="currentPassword" name="current_password" type="password" class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->updatePassword->get('current_password')" class="mt-2" />
                </div>

                <div class="py-2">
                    <x-input-label for="newPassword" :value="__('New Password')" />
                    <x-text-input id="newPassword" name="new_password" type="password" class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->updatePassword->get('new_password')" class="mt-2" />
                </div>

                <div class="py-2">
                    <x-input-label for="newPasswordConfirmation" :value="__('Confirm Password')" />
                    <x-text-input id="newPasswordConfirmation" name="new_password_confirmation" type="password" class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->updatePassword->get('new_password_confirmation')" class="mt-2" />
                </div>

                <div class="flex items-center gap-4 py-3">
                    <button class="crm-green-btn" type="button" id="changePassword">{{ __('Save') }}</button>
                </div>
            </form>
        </div>
    </div>
