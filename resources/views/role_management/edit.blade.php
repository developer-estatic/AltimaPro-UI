                                    ...
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12 col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="name" placeholder="{{ __('Name') }}" class="form-control" value="{{ $role->name }}">
                                                @error('name')
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="parent_id" class="form-control">
                                                    ...
                                                    @foreach($roles as $row)
                                                        <option value="{{ $row->id }}" {{ $row->id == $role->parent_id ? 'selected' : '' }}>{{ $row->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('parent_id')
                                                    ...
                                                    <div class="row">
                                                        @foreach($module->permissions as $permission)
                                                            <div class="col-md-3 py-2 mb-3">
                                                                <input type="checkbox" name="permission[{{$permission->id}}]" value="{{$permission->id}}" class="name" {{ in_array($permission->id, $rolePermissions) ? 'checked' : ''}}>
