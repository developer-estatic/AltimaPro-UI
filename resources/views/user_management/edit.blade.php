<section>
    ...
    <div class="crm-rightsidebar-header">
        <div class="crm-rightsidebar-header-left">
            <div class="crm-rightsidebar-header-title">Update User</div>
        </div>
    ...
    <div class="crm-rightsidebar-body form-row col-md-12">
        <div class="space-y-4">
            <form id="updateUserForm" action="{{ route('users.update', encrypt($user->id) ) }}" autocomplete="off">
                @csrf
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <x-text-input id="firstName" name="first_name" type="text" class="mt-1 block w-full" value="{{ old('first_name', $user->first_name) }}" placeholder="First Name" required autofocus autocomplete="new-first_name" />
                        <x-input-error class="mt-2" :messages="$errors->get('first_name')" />
                    </div>

                    <div class="col-md-6">
                        <x-input-label for="lastName" :value="__('Last Name')" />
                        <x-text-input id="lastName" name="last_name" type="text" class="mt-1 block w-full" value="{{ old('last_name', $user->last_name) }}" placeholder="Last Name" required autofocus autocomplete="new-last_name" />
                ...
                <div class="row mt-2">
                    <div class="col-md-6">
                        ...
                        <select id="roles" name="roles" class="form-control mt-1 block w-full">
                            ...
                            @foreach ($roles as $role)
                                <option value="{{ $role->name }}" {{ $role->name ==  old('roles', $user->roles[0]->name) ? 'selected' : '' }}>{{ $role->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <x-text-input id="email" name="email" type="text" class="mt-1 block w-full" value="{{ old('email', $user->email) }}" placeholder="example@mail.com" readonly required autofocus autocomplete="email" />
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-md-6">
                        ...
                        <x-text-input id="confirmPassword" name="password" type="password" class="mt-1 block w-full" minlength="4" placeholder="* * * * * *" autocomplete="off" />
                ...
                <div class="row mt-2">
                    <div class="col-md-6">
                        ...
                        <select id="country_id" name="country_id" class="form-control mt-1 block w-full">
                            ...
                            @foreach ($countries as $country)
                                <option data-code="{{$country->country_code}}" value="{{ $country->id }}" {{ $country->id ==  old('country_id', $user->country_id) ? 'selected' : '' }}>{{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full" value="{{ old('phone', $user->phone) }}" placeholder="xxxxx xxxxx" maxlength="12" required autofocus autocomplete="new-phone" />
                ...
                <div class="row mt-2">
                    <div class="col-md-6">
                        ...
                        <select id="business_unit_id" name="business_unit_id" class="form-control mt-1 block w-full">
                            ...
                            @foreach ($businessUnits as $businessUnit)
                                <option value="{{ $businessUnit->id }}" {{ $businessUnit->id ==  old('business_unit_id', $user->business_unit_id) ? 'selected' : '' }}>{{ $businessUnit->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="status" name="status" class="form-control mt-1 block w-full">
                            ...
                            {{-- <option value="SUSPENDED" {{ 'SUSPENDED' == $user->status ? 'selected' : '' }}>SUSPENDED</option> --}}
                        </select>
                    </div>
                </div>

                <div class="gap-4 py-5 text-center">
                    <button class="crm-green-btn w-full" type="button" id="updateUser">{{ __('Update') }}</button>
                </div>
